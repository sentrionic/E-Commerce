...
import (
	...
	"github.com/sentrionic/ecommerce/orders/handler"
	"github.com/sentrionic/ecommerce/orders/listeners"
...
func main() {
	...
	publisher := publishers.NewOrderPublisher(sc)
	...
	productListener := listeners.NewProductListener(sc, client)
	productListener.ProductCreatedListener(ctx)
	productListener.ProductUpdatedListener(ctx)

	expirationListener := listeners.NewExpirationListener(sc, client, publisher)
	expirationListener.ExpirationCompleteListener(ctx)

	paymentListener := listeners.NewPaymentListener(sc, client)
	paymentListener.PaymentCreatedListener(ctx)
