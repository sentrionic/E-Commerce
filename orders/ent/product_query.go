...
func (pq *ProductQuery) QueryOrder() *OrderQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2O, false, product.OrderTable, product.OrderColumn),
		)
...
func (pq *ProductQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Product, error) {
	...
	if query := pq.withOrder; query != nil {
		...
		for _, n := range neighbors {
			fk := n.product_order
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "product_order" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "product_order" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Order = n
		}
	}
