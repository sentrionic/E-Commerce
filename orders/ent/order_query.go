...
func (oq *OrderQuery) QueryProduct() *ProductQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2O, true, entorder.ProductTable, entorder.ProductColumn),
		)
...
func (oq *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if oq.withProduct != nil {
		withFKs = true
	}
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, entorder.ForeignKeys...)
	}
	...
	if query := oq.withProduct; query != nil {
		...
		for i := range nodes {
			if nodes[i].product_order == nil {
				continue
			}
			fk := *nodes[i].product_order
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "product_order" returned %v`, n.ID)
			}
			for i := range nodes {
